...
def gpu_grad_nll(gains, data, scale, phs_norm_fac, noise, diff_mat, src_mat, edges, ant_1_array, ant_2_array, xp):
    ...
    #5. compute s and t => DONT NEED ANYMORE 
    ...
    p = undo_zeroPad(p, edges, ReImsplit=True)
    q = undo_zeroPad(q, edges, ReImsplit=True)
    ...
    # assert(p.shape == gains.shape)

    # gradr, gradi = populate_gradient(
    #     n_ant, gains, p, q, P, noise, ant_1_array, ant_2_array
    # )

    gradr, gradi = populate_gradient(
        n_ant, gains, pr, pi, qr, qi, P, noise, ant_1_array, ant_2_array
    )
...
def populate_gradient(n_ant, gains, pr, pi, qr, qi, P, noise, ant_1_inds, ant_2_inds):
    ...
    # even_1_inds, even_2_inds = 2*ant_1_inds, 2*ant_2_inds
    # odd_1_inds, odd_2_inds = 2*ant_1_inds+1, 2*ant_2_inds+1
    
    # ant_1_inds_2d = cp.concatenate((even_1_inds, odd_1_inds, even_1_inds, odd_1_inds))
    # ant_2_inds_2d = cp.concatenate((even_2_inds, odd_2_inds, odd_2_inds, even_2_inds))

    # n_ant = len(gains)
    pop_grad_lib.populate_gradient(
        ctypes.cast(gains.data.ptr, ctypes.POINTER(ctypes.c_double)),
        ctypes.cast(pr.data.ptr, ctypes.POINTER(ctypes.c_double)),
        ctypes.cast(pi.data.ptr, ctypes.POINTER(ctypes.c_double)),
        ctypes.cast(qr.data.ptr, ctypes.POINTER(ctypes.c_double)),
        ctypes.cast(qi.data.ptr, ctypes.POINTER(ctypes.c_double)),
...
def simulate(n_ant, debugging=False):
    ...
    # return np.allclose(gpu_grad, cpu_grad)

    # print(cpu_grad.shape)
    # print(gpu_grad.shape)

    # assert(np.allclose(gpu_grad, cpu_grad))

if __name__ == "__main__":
    #simulate test matrices to test intermitantly as we go along above
    # n_ant = 200
    # n_ant_array = np.array([100, 200, 300])
    # for n_ant in n_ant_array:
    #     results_n_ant = []
    #     for i in range(2):
    #         result = simulate(n_ant = n_ant, debugging=False)
    #         result = cp.asnumpy(result)
    #         results_n_ant.append(result)
    #         # print(type(result))
    #         print(f"on trial {i}")
    #     # print(results_n_ant_512)
    ...
    #     plt.savefig('Results_for_nant={}.png'.format(n_ant), dpi=300, format='png', bbox_inches='tight')
    #     # plt.show()
